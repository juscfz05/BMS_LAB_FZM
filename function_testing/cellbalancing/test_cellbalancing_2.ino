...
void checkcellbalancing(float VCell[]);
...
void loop() {
  ...
  if (setbalActive > 0 && counter == 1) {
    unsigned long currentMillisStop = millis();
    unsigned long Diff = currentMillisStop - currentMillisStart;
    counter = 2;
    Serial.print("Time till balancing active: ");
    Serial.println(Diff);
  } else if (setbalActive == 0 && counter == 2){
...
void checkcellbalancing(float VCell[4])
{
  ...
  const long cellbalcalcInterval = 100;               // konstantes Zeitintervall (200 ms) in dem die Cell-Balancing-Berechnung durchgeführt wird
