...
bool WarningUndervoltage = true;
bool WarningOvervoltage = false;           
bool WarningOvertemp = false;
float Ibat = 350;

unsigned long StrtUVWarnOff = 0;
...
void loop() {
  if (millis() >= 3000 && WarningUndervoltage == true) {
    WarningUndervoltage = false;
    StrtUVWarnOff = millis();
    //Serial.println((String)"Start time of undervoltage warning deactivated test: "+StrtUVWarnOff);
  }

  SafetyController(WarningOvervoltage, WarningUndervoltage, WarningOvertemp, Ibat);

  if (BDUtrigger == true) {
    static unsigned long EndUVWarnOff = millis();
    //Serial.println((String)"End time of undervoltage warning deactivated test: "+EndUVWarnOff);
    unsigned long DiffUVWarnOff = EndUVWarnOff - StrtUVWarnOff;
    Serial.println((String)"Difference time of undervoltage warning deactivated test: "+DiffUVWarnOff);
    delay(100000);
  }
}
