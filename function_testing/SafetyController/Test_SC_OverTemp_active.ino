...
unsigned long StrtOTempWarnOn = 0;
...
void loop() {
  if (millis() >= 3000 && WarningOvertemp == false) {
    WarningOvertemp = true;
    StrtOTempWarnOn = millis();
    //Serial.println((String)"Start time of over temperature warning activated test: "+StrtOTempWarnOn);
  }

  SafetyController(WarningOvervoltage, WarningUndervoltage, WarningOvertemp, Ibat);

  if (BDUtrigger == true) {
    static unsigned long EndOTempWarnOn = millis();
    //Serial.println((String)"End time of over temperature activated test: "+EndOTempWarnOn);
    unsigned long DiffOTempWarnOn = EndOTempWarnOn - StrtOTempWarnOn;
    Serial.println((String)"Difference time of over temperature warning activated test: "+DiffOTempWarnOn);
    delay(100000);
  }
}
