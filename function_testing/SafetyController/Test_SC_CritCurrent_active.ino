...
unsigned long StrtIbatWarnOn = 0;
...
void loop() {
  if (millis() >= 5500 && Ibat <= 400) {
    Ibat = 450;
    StrtIbatWarnOn = millis();
    //Serial.println((String)"Start time of critical current activated test: "+StrtIbatWarnOn);
  }

  SafetyController(WarningOvervoltage, WarningUndervoltage, WarningOvertemp, Ibat);

  if (BDUtrigger == true) {
    static unsigned long EndIbatWarnOn = millis();
    //Serial.println((String)"End time of critical current activated test: "+EndIbatWarnOn);
    unsigned long DiffIbatWarnOn = EndIbatWarnOn - StrtIbatWarnOn;
    Serial.println((String)"Difference time of critical current activated test: "+DiffIbatWarnOn);
    delay(100000);
  }
}
