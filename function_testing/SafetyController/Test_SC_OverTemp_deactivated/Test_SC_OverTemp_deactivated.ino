...
void loop() {
  if (millis() >= 3000 && WarningOvertemp == true) {
    WarningOvertemp = false;
    StrtOTempWarnOff = millis();
    //Serial.println((String)"Start time of over temperature warning deactivated test: "+StrtOTempWarnOff);
  }

  SafetyController(WarningOvervoltage, WarningUndervoltage, WarningOvertemp, Ibat);

  if (BDUtrigger == false && WarningOvertemp == false) {
    static unsigned long EndOTempWarnOff = millis();
    //Serial.println((String)"End time of over temperature deactivated test: "+EndOTempWarnOff);
    unsigned long DiffOTempWarnOff = EndOTempWarnOff - StrtOTempWarnOff;
    Serial.println((String)"Differnce time of over temperature deactivated test: "+DiffOTempWarnOff);
    delay(100000);
  }
}
