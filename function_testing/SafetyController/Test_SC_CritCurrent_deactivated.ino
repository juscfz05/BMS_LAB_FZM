...
unsigned long StrtIbatWarnOff = 0;
...
void loop() {
  if (millis() >= 3000 && Ibat > 400) {
    Ibat = 350;
    StrtIbatWarnOff = millis();
    //Serial.println((String)"Start time of critical current deactivated test: "+StrtIbatWarnOff);
  }

  SafetyController(WarningOvervoltage, WarningUndervoltage, WarningOvertemp, Ibat);

  if (BDUtrigger == false && Ibat <= 400) {
    static unsigned long EndIbatWarnOff = millis();
    //Serial.println((String)"End time of critical current deactivated test: "+EndIbatWarnOff);
    unsigned long DiffIbatWarnOff = EndIbatWarnOff - StrtIbatWarnOff;
    Serial.println((String)"Difference time of critical current deactivated test: "+DiffIbatWarnOff);
    delay(100000);
  }
}
