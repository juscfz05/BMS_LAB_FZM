...
bool WarningOvervoltage = true;
bool WarningUndervoltage = false;           
bool WarningOvertemp = false;
float Ibat = 350;

unsigned long StrtOVWarnOff = 0;
...
void loop() {
  if (millis() >= 3000 && WarningOvervoltage == true) {
    WarningOvervoltage = false;
    StrtOVWarnOff = millis();
    //Serial.println((String)"Start time of overvoltage warning deactivated test: "+StrtOVWarnOff);
  }

  SafetyController(WarningOvervoltage, WarningUndervoltage, WarningOvertemp, Ibat);

  if (BDUtrigger == true) {
    static unsigned long EndOVWarnOff = millis();
    //Serial.println((String)"End time of overvoltage warning deactivated test: "+EndOVWarnOff);
    unsigned long DiffOVWarnOff = EndOVWarnOff - StrtOVWarnOff;
    Serial.println((String)"Difference time of overvoltage warning deactivated test: "+EndOVWarnOff);
    delay(10000);
  }
}
