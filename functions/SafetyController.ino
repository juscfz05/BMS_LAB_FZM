...
{ 
  ...
  if (currentMillis - previousMillis_SC >= Interval_SC){                     // nach Zeitintervall 50 ms werden Warnungen abgefragt
    ...
    if (WarningOvervoltage == true && WarnOvervolt_count < 12) {             // WarnOvervolt_count hochzählen wenn Byte regWarnings Bit 0 = true (Überspannung erkannt) und Zähler < 12
      WarnOvervolt_count++;
      NoWarnOvervolt_count = 0;
    } else if (WarningOvervoltage == false && NoWarnOvervolt_count < 12){    // NoWarnOvervolt_count hochzählen wenn Byte regWarnings Bit 0 = false (keine Überspannung erkannt) und Zähler < 12
      NoWarnOvervolt_count++;
      WarnOvervolt_count = 0;
    }

    if (WarningUndervoltage == true && WarnUndervolt_count < 12) {            // WarnUndervolt_count hochzählen wenn Byte regWarnings Bit 1 = true (Unterspannung erkannt) und Zähler < 12
      WarnUndervolt_count++;
      NoWarnUndervolt_count = 0;
    } else if (WarningUndervoltage == false && NoWarnUndervolt_count < 12) {  // NoWarnUndervolt_count hochzählen wenn Byte regWarnings Bit 1 = false (keine Unterspannung erkannt) und Zähler < 12
      NoWarnUndervolt_count++;
      WarnUndervolt_count = 0;
    }

    if (WarningOvertemp == true && WarnOvertemp_count < 12) {                 // WarnOvertemp_count hochzählen wenn Byte regWarnings Bit 2 = true (Übertemperatur erkannt) und Zähler < 12
      WarnOvertemp_count++;
      NoWarnOvertemp_count = 0;
    } else if (WarningOvertemp == false && NoWarnOvertemp_count < 12){        // NoWarnOvertemp_count hochzählen wenn Byte regWarnings Bit 2 = false (keine Übertemperatur erkannt) und Zähler < 12
      NoWarnOvertemp_count++;
      WarnOvertemp_count = 0;
    }
    
    if (Ibat_200ms > 400 && WarnIbat_count < 12) {                                  // WarnIbat_count hochzählen wenn Ibat > 400 A und Zähler < 12
      WarnIbat_count++;
      NoWarnIbat_count = 0;
    } else if (Ibat_200ms <= 400 && NoWarnIbat_count < 12) {                        // NoWarnIbat_count hochzählen wenn Ibat <= 400 A und Zähler < 12
      NoWarnIbat_count++;
      WarnIbat_count = 0;
    }
    ...
    if (WarnOvervolt_count > 11 || WarnUndervolt_count > 11 || WarnOvertemp_count > 11 || WarnIbat_count > 11 || (NoWarnOvervolt_count < 11 && NoWarnUndervolt_count < 11 &&  NoWarnOvertemp_count < 11 && NoWarnIbat_count < 11)) { // Abfrage ob eine der Warnungen länger als 600 ms (50*12) angehalten hat oder die NoWarnings noch nicht länger als 600 ms voriegen
      BDUactive = true;    // Bit Schütze öffnen wenn eine Warnung >= 600 ms anliegt oder alle der NoWarnungen < 600 ms anliegen
      ...
      BDUactive = false;   // Bit Schütze schließen wenn für >= 600 ms keine Warnung anliegt oder eine Warnung noch < 600 ms anliegt
    } 
  }
}