void showMeasurementValues(float Vcell[4], float Tcell[4], int Ibat, bool WarningOvertemp, bool WarningOvervolt, bool WarningUndervolt)
{
  ...
  for (i = 1; i <= 4; i++) // Schleife mit i 1-4 für die jeweiligen Zellen
  {
    ...
    writeText(i*68-66, 22, 2, String(Vcell[i-1],2), colCell(i)); // Anzeige der Zellspannungen für jeweilige Zelle
    writeText(i*68-66, 40, 2, strLen(String(Tcell[i-1],1),4), colTemp(i)); // Anzeige der Zelltemperatur für jeweilige Zelle
  }

  // write pack current on screen
  writeText(280,  4, 2, "I/A",ILI9341_LIGHTGREY); // Anzeige Strom in Ampere
  writeText(270, 22, 2, strLen(String(Ibat),4),ILI9341_BLUE); // Anzeigen der derzeitigen Stromstärke
  ...
  // Warnungen
  if (WarningOvervolt == true) // Prüfen ob Überspannung vorliegt
  ...
  if (WarningUndervolt == true) // Prüfen ob Unterspannung vorliegt
  ...
  if (WarningOvertemp == true) // Prüfen ob Übertemperatur in einer Zelle vorliegt
...
void drawMeasurementCurves(uint16_t fullScreenDuration, float Vcell[4], float Tcell[4], int Ibat)
{
  ...
  for (i = 1; i <= 4; i++)
  {
    if ((Vcell[i-1] > 4.2) || (Vcell[i-1] < 2.5)) // check if Cell Voltage limit is crossed
      drawPixel(tx/tS, yV-dV*max(min(Vcell[i-1],4.5),2.2), ILI9341_RED); // draw critical Cell Voltages red
    else
      drawPixel(tx/tS, yV-dV*max(min(Vcell[i-1],4.5),2.2), colCell(i)); // draw Cell Voltages within the limits

    if (Tcell[i-1] > 60) // check if Cell Temperature is crossed
      drawPixel(tx/tS, yT-dT*Tcell[i-1], ILI9341_RED); // draw critical Cell Temperatures red
    else
      drawPixel(tx/tS, yT-dT*Tcell[i-1], colTemp(i)); // draw Cell Temperatures within the limits
  }
  // draw Current
